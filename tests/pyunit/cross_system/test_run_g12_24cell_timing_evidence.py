...
class RunG1224CellTimingEvidenceTest(unittest.TestCase):
    def setUp(self):
        ...
        self.identity = runner.CampaignIdentity(
            ...
            input_manifest_path=str(self.root / "inputs.json"),
            input_manifest_sha256=_digest("inputs"),
            prepared_manifest_path=str(self.root / "registry.json"),
    ...
    def _registry(self, scale=12):
        graph = self._file_record(f"graph/g{scale}.sg", f"g{scale} graph".encode())
        return {
    ...
    def _write_registry(self, scale=12):
        path = self.root / f"registry-{scale}.json"
        path.write_text(
            json.dumps(self._registry(scale=scale)) + "\n",
            encoding="utf-8",
        )
        return path

    def _input_manifest(self, registry):
        workloads = {}
        for workload in evidence.WORKLOADS:
            source = registry["cells"][f"{workload}:200ns"]
            workloads[workload] = {
    ...
    def test_input_manifest_must_match_registry(self):
        ...
        with self.assertRaisesRegex(
            runner.CampaignError,
            "G12 input manifest differs from registry",
        ):
            runner.validate_input_manifest(inputs, registry)

    def test_resume_rejects_changed_preparer_hash(self):
        state = runner.new_state(self.identity)
        changed = dataclasses.replace(
            self.identity,
            registry_preparer_sha256=_digest("changed-preparer"),
        )
        with self.assertRaisesRegex(runner.CampaignError, "identity differs"):
            runner.resume_state(state, changed)
    ...
    def test_interrupted_running_stage_uses_next_attempt(self):
        ...
        runner.execute_replay_stage(
            ...
            replay_launcher=launch,
            state_lock=threading.Lock(),
        )
        self.assertEqual(
            state["cells"][key]["stages"]["host_inline"]["attempt"],
            2,
        )
