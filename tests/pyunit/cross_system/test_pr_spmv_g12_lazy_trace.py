...
from scripts import pr_spmv_g12_lazy_trace as pagerank
...
class PageRankG12LazyTraceTest(unittest.TestCase):
    def setUp(self):
    ...
    def test_bundle_separates_graph_and_initial_state_identities(self):
        ...
        self.assertEqual(bundle.meta["graph_scale"], 12)
        self.assertEqual(bundle.meta["iterations"], 20)
        self.assertEqual(bundle.meta["graph_sha256"], digest("g12.sg"))
        self.assertEqual(
    ...
    def test_zero_degree_vertex_has_finite_zero_contribution(self):
        bundle = pagerank.build_bundle(
            ...
            source_sha256=digest("source"),
            binary_sha256=digest("binary"),
            config_sha256=digest("config"), graph_scale=12,
    ...
    def test_build_from_csr_rejects_graph_hash_drift(self):
        ...
        (csr / "graph.meta.json").write_text(
            ...
            '"num_directed_edges":4,"graph_sha256":"' + digest("g12.sg")
            + '"}\n', encoding="utf-8",
        )
        graph = self.root / "g12.sg"
        graph.write_bytes(b"g12.sg")
        with self.assertRaisesRegex(
            pagerank.PageRankTraceError, "G12 graph identity differs"
        ):
    ...
    def test_g12_sized_descriptor_stays_bounded(self):
        import tracemalloc
