    ...
    for latency, vanilla_ticks in BASE_TICKS.items():
        for system, divisor in (
            ...
            row.update(
                ...
                amu_logical_values="192" if is_amu else "0",
                amu_line_requests="64" if is_amu else "0",
                amu_line_cache_hits="16" if is_amu else "0",
                amu_coalesced_misses="112" if is_amu else "0",
    ...
    for latency, base_ticks in BASE_TICKS.items():
        ...
        for system, divisor in (
            ...
            write_csv(summary, {
                ...
                "amu_logical_values": "192" if kind == "amu" else "0",
                "amu_line_requests": "64" if kind == "amu" else "0",
                "amu_line_cache_hits": "16" if kind == "amu" else "0",
                "amu_coalesced_misses": "112" if kind == "amu" else "0",
                "cira_prefetches": "64" if is_cira else "0",
                "cira_completed": "64" if is_cira else "0",
                "cira_indexed_prefetches": "0",
                "cira_csr_prefetches": "16" if is_cira else "0",
