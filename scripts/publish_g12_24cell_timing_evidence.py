#!/usr/bin/env python3
"""Publish deterministic G12 progress or final host/CIRA timing artifacts."""
...
try:
    ...
    import run_g12_24cell_timing_evidence as runner
    import timing_evidence_24cell as evidence


FIELDS = (
    "workload", "latency", "graph_scale", "input_sha256",
    "host_status", "cira_status", "host_over_cira_speedup",
    ...
    "cira_issued_prefetches", "cira_completed_prefetches",
    ...
    "pr_compute_plus_stall_max_ticks",
...
def _validate_cira(cira: dict, key: str) -> None:
    ...
    issued_total = cira.get("issued_prefetches")
    completed_total = cira.get("completed_prefetches")
    if (
        issued != completed
        or any(value == 0 for value in issued)
        or isinstance(issued_total, bool)
        or not isinstance(issued_total, int)
        or isinstance(completed_total, bool)
        or not isinstance(completed_total, int)
        or issued_total != completed_total
        or issued_total != sum(issued)
    ):
...
def _row(
    ...
    *, graph_scale: int, input_sha256: str,
) -> dict:
    row = {field: "" for field in FIELDS}
    row.update({
        "workload": workload, "latency": latency,
        "graph_scale": graph_scale, "input_sha256": input_sha256,
    ...
    cira = values["cira"]
    if host is not None and cira is not None:
        row["host_over_cira_speedup"] = _speedup(
            host["host_region_cumulative_ticks"],
            cira["generic_prefetch"]["busy_ticks"],
        )
    if cira is not None:
        ...
        row.update({
            ...
            "cira_issued_prefetches": cira["issued_prefetches"],
            "cira_completed_prefetches": cira["completed_prefetches"],
            "pr_descriptor_applicable": "false",
            "pr_compute_ticks": descriptor["compute_ticks"],
            "pr_queue_stall_ticks": descriptor["queue_stall_ticks"],
            "pr_compute_plus_stall_max_ticks": max(
                compute + stall for compute, stall in zip(
                    descriptor["compute_ticks_per_core"],
                    descriptor["queue_stall_ticks_per_core"],
                )
            ),
        })
        for core in range(4):
...
def _readme(rows: list[dict], progress: bool) -> str:
    title = "G12 host-inline and CIRA timing progress" if progress else (
...
def publish(source: Path, destination: Path, *, progress: bool = False) -> Path:
    ...
    input_path = Path(identity.input_manifest_path).resolve()
    prepared_path = Path(identity.prepared_manifest_path).resolve()
    if evidence.sha256_file(input_path) != identity.input_manifest_sha256:
        raise PublishError("input manifest SHA-256 differs")
    if evidence.sha256_file(prepared_path) != identity.prepared_manifest_sha256:
        raise PublishError("prepared manifest SHA-256 differs")
    input_manifest = _load_json(input_path, "G12 input manifest")
    prepared_manifest = _load_json(prepared_path, "G12 prepared manifest")
    input_graph = input_manifest.get("graph")
    prepared_graph = prepared_manifest.get("graph")
    if (
        not isinstance(input_graph, dict)
        or not isinstance(prepared_graph, dict)
        or input_graph.get("scale") != 12
        or prepared_graph.get("scale") != 12
    ):
        raise PublishError("source campaign is not G12")
    try:
        runner.validate_input_manifest(input_manifest, prepared_manifest)
    except runner.CampaignError as error:
        raise PublishError(
            f"G12 input manifest differs from prepared manifest: {error}"
        ) from error
    expected = {
    ...
    for workload, latency, values in validated:
        ...
        rows.append(_row(
            workload, latency, values, output_records,
            graph_scale=prepared_graph["scale"],
            input_sha256=input_manifest["workloads"][workload][
                "input_sha256"
            ],
        ))
    ...
    manifest = {
        ...
        "graph_scale": prepared_graph["scale"],
        "graph_sha256": prepared_graph["sha256"],
        "input_manifest": {
            "path": str(input_path),
            "sha256": identity.input_manifest_sha256,
        },
        "prepared_manifest": {
            "path": str(prepared_path),
            "sha256": identity.prepared_manifest_sha256,
        },
...
def main(argv=None) -> int:
    ...
    try:
        ...
        print(f"G12_TIMING_24CELL_PUBLISH_FAILED {error}")
        return 1
    return 0
