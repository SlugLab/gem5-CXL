...
#ifndef GAPBS_AMU_STACK_PRIME_BYTES
#define GAPBS_AMU_STACK_PRIME_BYTES (64 * 1024)
#endif

namespace gapbs_amu {

template <typename GraphT>
static inline void prime_graph_pages(const GraphT &g)
{
...
static inline void prime_worker_stack_pages()
{
#pragma omp parallel
  {
    volatile unsigned char stack_pages[GAPBS_AMU_STACK_PRIME_BYTES];
    for (size_t offset = 0; offset < GAPBS_AMU_STACK_PRIME_BYTES;
         offset += 4096)
      stack_pages[offset] = 0;
    stack_pages[GAPBS_AMU_STACK_PRIME_BYTES - 1] = 0;
    __asm__ volatile("" : : "r"(stack_pages) : "memory");
  }
}
...
static inline void flush_source_lines(const void *addr, size_t size)
{
  const uintptr_t first = reinterpret_cast<uintptr_t>(addr) & ~uintptr_t(63);
  const uintptr_t last =
      (reinterpret_cast<uintptr_t>(addr) + size - 1) & ~uintptr_t(63);
...
static inline T load_value(const T *addr)
{
  ...
  flush_source_lines(addr, sizeof(T));
 ...
 public:
  ...
  void issue_all()
  {
    ...
    for (size_t i = 0; i < count_; ++i)
      flush_source_lines(addrs_[i], sizes_[i]);
    for (size_t i = 0; i < count_; ++i)
